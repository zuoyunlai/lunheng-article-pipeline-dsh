...
4. 本地跑**四道门**，全绿才提交：
   ...
   ```
5. 提交并推送分支；
6. **发布 = 只推 tag**：`git tag v18.0.4 && git push origin v18.0.4`（tag 必须等于 `v` + `package.json.version`，publish 工作流会校验；v18.0.5 更正：本节旧版把推送目标写成 `v18.0.0`，照抄会推错 tag）
...
```sh
git tag v18.0.5 && git push origin v18.0.5   # 工作流会校验 tag == v + package.json.version
```
