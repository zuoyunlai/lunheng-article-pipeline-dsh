...
5. 提交并推送分支；
6. **发布 = 只推 tag**：`git tag v18.15.0 && git push origin v18.15.0`（tag 必须等于 `v` + `package.json.version`，publish 工作流会校验；**v18.2.1 更正：本行示例上一版停在 `v18.0.4`——bump 脚本的点位正则按行首锚定，扫不到这种内联形态，两次都漏了**；**v18.2.2 更正：第三处人工刷新**；**v18.2.3 更正：第四处人工刷新 —— 根因与终结方案见 §版本号约定 的「已知漏点」注**；**v18.2.4 起已机械化**：`consistency-check` 规则①/⑦ 同址补扫本形态，每次 bump 漏刷即 P1 变红）
...
```sh
git tag v18.15.0 && git push origin v18.15.0   # 工作流会校验 tag == v + package.json.version
```
