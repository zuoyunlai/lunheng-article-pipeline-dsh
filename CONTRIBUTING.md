...
4. 本地跑**四道门**，全绿才提交：
   ...
   ```
5. 提交并推送分支；
6. **发布 = 只推 tag**：`git tag v18.0.3 && git push origin v18.0.0`（tag 必须等于 `v` + `package.json.version`，publish 工作流会校验）
...
```sh
git tag v18.0.3 && git push origin v18.0.0   # 工作流会校验 tag == v + package.json.version
```
