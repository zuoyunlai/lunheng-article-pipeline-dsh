...
> **⚠️ 已知漏点：内联 `git tag vX.Y.Z` 形态不在任何机械门的扫描面内（v18.2.2 记录，v18.2.3 更新）**——`consistency-check.mjs` 的四条版本规则各有明确锚点：①/⑦ 认 `> 版本：` 开头行、⑫ 认 `^\s*[-*>#]*\s*版本：`、⑧ 认 `cordis.patch.yml` 头与 `examples/` 的 `@x.y.z` **安装 pin**、⑬ 认 `docs/` 的安装 pin 与「当前版本」行。而 **`git tag v18.2.3 && git push origin v18.2.3` 这类内联示例**既非 `版本：` 前缀、也非安装 pin → **四条规则全都扫不到**。
> **代价**：`CONTRIBUTING.md`（本节下方「升级流程」步 6 + 「发布（维护者）」段）与 5 语 README 各有一处该形态，**每次 bump 都靠人工记得刷**——截至 v18.2.3 已**漏过四次**（`v18.0.4` → `v18.2.1` 人工修正一次 → `v18.2.2` 人工修正一次 → `v18.2.3` 人工修正一次）。
> **建议的终结方案**（尚未实施）：在 `consistency-check.mjs` 增一条规则扫 `git tag v?<SEMVER>` 内联形态（判 P1），并在 `references/_shared/规范-机械门对照表.md` 登记该规范。**未随 v18.2.2 / v18.2.3 实施的原因**：属**新增门**而非修既有门，会改变门数与失败面，宜与门数表述（`SKILL.md`/`AGENTS.md` 受词预算棘轮约束）一并处理。
...
4. 本地跑**四道门**，全绿才提交：
   ...
   ```
5. 提交并推送分支；
6. **发布 = 只推 tag**：`git tag v18.2.3 && git push origin v18.2.3`（tag 必须等于 `v` + `package.json.version`，publish 工作流会校验；**v18.2.1 更正：本行示例上一版停在 `v18.0.4`——bump 脚本的点位正则按行首锚定，扫不到这种内联形态，两次都漏了**；**v18.2.2 更正：第三处人工刷新**；**v18.2.3 更正：第四处人工刷新 —— 根因与终结方案见 §版本号约定 的「已知漏点」注**）
...
```sh
git tag v18.2.3 && git push origin v18.2.3   # 工作流会校验 tag == v + package.json.version
```
