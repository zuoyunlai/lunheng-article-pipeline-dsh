...
# 锁定具体版本：dsh plugin --profile web add lunheng-article-pipeline@18.18.11

# 2. （可选，但要让三档工具真的挂上就必做）设三档 subagent 工具的 provider/model 环境变量
#    v18.2.6 起：不设任何变量 = 三档行**根本不装载**（不再是「装载但全继承」）
#    ——三档全继承时它们与内置 subagent 完全同义，无条件装载等于每会话白付 3 份工具 schema。
#    设了任一档的 PROVIDER/MODEL，或 LUNHENG_TIERING=on，才装载三行。
...
```sh
# ① patch 行进入组合树：4 段 insert 应全部命中（第 1 段是自注册行，必须恰一行）
...
> ⚠️ **`--dump-config` 只打印声明行**：`disabled: !!js` 的行级门控**不会**在这里体现为「行消失」——所以「dump 里看得到三行」≠「三行已被装载」。要确认是否装载，看第 1 步的变量是否设了（或设 `LUNHENG_TIERING=on` 做一次显式开启对照）。

## 3. preset.yml 仅是分档说明文件

`preset.yml` 不是 DSH 加载的 cordis 配置——它**只是给主人读的分档说明**。
真要装分档行为，看 `../../cordis.patch.yml`（包根）里的 3 档 subagent 工具，**并显式开启**（不设任何 `LUNHENG_*` 时默认不挂；设任一档 `*_PROVIDER`/`*_MODEL` 或 `LUNHENG_TIERING=on` 才装载）。

```sh
# preset.yml 的内容（原文是一行 description，此处按行摘要）：
# name: 论衡分档
# description: 论衡专用预设：三档 subagent 工具按角色能力分模型（检索 T1-T3 便宜快 / 分析写作 T4-T5 推理强 /
#              批判审计 T6+T7+T9+G14 顶配防漏判），模型经 LUNHENG_*_PROVIDER + LUNHENG_*_MODEL 环境变量覆盖。
#              **v18.2.6 起默认不挂载**：设任一档 *_PROVIDER/*_MODEL 或 LUNHENG_TIERING=on 才装载。
# 真源：references/_shared/模型路由.md + scripts/model-routing.mjs + 包根 cordis.patch.yml 的「分档闸门」段
```
...
| 环境变量组合 | 结果 |
|---|---|
| 都不设 | 三行**不装载**（v18.2.6 起）→ 派发走内置 `subagent`，继承会话模型（不报错，最省心，也是默认） |
| `LUNHENG_TIERING=off` | 强制**不装载**（优先级最高：即便其它 `LUNHENG_*` 已设也关掉） |
| `LUNHENG_TIERING=on` | **装载**三行但不指定模型 → 三档全继承（用于确认工具是否可见） |
| 只设 `MODEL` | 装载且**生效**（字段独立：`provider` 由宿主逐字段继承父级） |
| 只设 `PROVIDER` | 装载且生效（不写 `model` 时用该 provider 的默认模型） |
...
- **删掉的东西**：早期版本第一段 `- insert:` 行挂 `@deepseek-ai/dsh-skill-filesystem` 提供者
  ...
  且不再依赖内部包名 `@deepseek-ai/dsh-skill-filesystem`。
- **本包现在的 6 处 `!!js`**（v18.2.6 更正计数）：**3 处 `agentOptions`**（三段分档工具行，见第 4 节）+ **3 处 `disabled`**（同一批行的行级门控，读 `LUNHENG_TIERING` 与三档 `*_PROVIDER`/`*_MODEL`）。`disabled` **同样由宿主在加载期以完整 Node 权限求值**（`@deepseek-ai/cordis-plugin-loader/src/config/entry.ts:104-107` 的 `disabledOf()`），故**加载期执行面是 6 处而非旧版写的 3 处**。两者都只用 `process.env.*` 与全局 `Object.assign`。
...
## 6. 可选：把三档工具行移进 agent preset（v18.1.0 新增配方，**未在真实部署验证**）

上面的 3 步是**默认路径**：三行 `- insert:` 在**包级** `cordis.patch.yml`，因此**该 profile 的每个会话**（装载时）都能看到三个分档工具。
...
# 卸载后 4 段 - insert 行（自注册行 + 三档工具行）与入口注册的技能一并消失；
