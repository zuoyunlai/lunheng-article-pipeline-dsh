...
> **v2.5.2-dsh.7 重大变更**：早期版本的 `agent.cordis.yml`（≈16KB standard 全量副本）**已删除**——
...
dsh plugin --profile web add lunheng-article-pipeline@2.5.2-dsh.7
...
注意：v2.5.2-dsh.7 起**未设环境变量直接抛错**（旧版本会 silent fallback，可能配错 model 不知道）。
...
要点：
- `providerName: skill-filesystem-lunheng`（v2.5.2-dsh.7 命名规范以 `skill-filesystem-*` 前缀区分系统默认）——这是论衡自己的提供者 namespace，与 system 默认 `filesystem` 不冲突
...
```sh
dsh plugin --profile web update lunheng-article-pipeline@2.5.2-dsh.7
