...
## 为什么第三方插件市场（如 dshmarket）里显示「安装完成但校验失败 / 入口产物缺失」？

> **v18.0.0 起已不适用**：本包现在带 JS 包入口（`main` → `lib/index.js`，注册技能并经 `ctx.effect` 可逆注册），「只认 JS 入口」一类校验器不再误报。若仍看到该校验失败，请确认安装的是 v18.0.0 及以上。
>
> **先分清归属（v18.2.4 补注）**：`dshmarket` 与各类插件市场都是**第三方社区项目**，不是 DSH 官方组件，也不在本包的依赖或支持范围内——本包只依赖 DSH 宿主本身（且 `@deepseek-ai/dsh` 是 optional peer）。故「市场里的显示」与「本包能否装载」是两件事：前者出问题找市场，后者出问题看本文件与 [`troubleshooting.md`](troubleshooting.md)。
...
| 场景 | 版本 |
|---|---|
| 本轮迭代 | 抬 minor（`18.2.0` 形态） |
| 上一轮迭代（未单独发布，内容随本版首发） | 抬 major（`17.0.0` 形态） |
| 同一轮内修补 | 抬 patch（`18.2.4` 形态） |
| 同一轮内小步改进 | 抬 patch（同上） |

> **表中数字只是示例**（v18.2.4 更正）：旧版此处写死成当时的版本 `18.2.1`，此后每次 bump 都不同步、静默滞留了三个版本——**「当前版本」的唯一真源是 `package.json` 的 `version`**（也可 `npm view lunheng-article-pipeline version`）。本文件刻意不再复述具体当前版本号。
