...
> 版本：v18.18.14（DSH 原生插件，发布于 2026-09-25；详见 CHANGELOG）
...
4. **同名覆盖**：检查当前工作目录下是否存在 `.dsh/skills/lunheng-article-pipeline/`（项目技能根 rank 100 **高于**本包的 rank 250，会**静默顶替**）。删掉或改名该目录即可确认。
...
| 40 | **`md2html.mjs`（v18.12.0）**：导出被拒 / `--strict` 校验失败（SVG 结构不合格、缺图、外部引用等）—— 对策：按告警补图件或改 SVG；**不是** M 门的「2 = P0」，不要改正文 |
| 20 | **交接门 `handoff-check.mjs`（v18.6.0）**：产物缺失或 0 字节 —— 对策：重派（或按快照续作） |
| 21 | **交接门 `handoff-check.mjs`（v18.6.0）**：结构 / 版本 / 成对 / 回报段不合 —— 对策：续接补交 |
| 22 | **交接门 `handoff-check.mjs`（v18.6.0）**：仅软提示 —— 对策：人工复核后放行 |
...
>
> ⚠️ **v18.12.0/v18.12.1 修掉的三处字号/口径误导**：
> 6. **`token-budget` / `token-cost` 把「用法/参数错」记成 1**：`--bogus`（拼错）、`--top 0`、未给模式、会话缓存不存在……旧版一律 `exit 1`，而 `1` = 「P1 内容失败」→ 主控按 M 门语义读码会**误判为正文有 P1 残留并触发 T5 修订轮**（这两个脚本根本不做内容判定）。现整份契约**不再出现 1**：一律 `10`（参数或路径错）。（同批 `model-routing` 的用法/配置错同改。）
> 7. **`EXIT_CONTRACT` 只覆盖 13/23**：退出码表靠人工维护，新增脚本「装了 guard 却忘了登记」不会有任何门发现。现补齐 **23/23**，并把「凡真 import `_lib/exit-guard.mjs` 的随包脚本必须登记（豁免须在 `EXIT_GUARDED_EXEMPT` 写明理由）」写成判据——补表时该判据当场抓出 `fix-gates`（表外码 1）与 `meta-synthesize`（表内幽灵码 1）两处。
> 8. **`md2html` 用 `2` 表示「导出被拒 / `--strict` 校验失败」**：`2` 在本表里是「存在 P0 失败」→ 复核者会把「因缺图拒绝导出」读成「定稿有 P0」，而补救动作完全不同（补图件 vs 改正文）。现改用独立码 **`40`**（同一族：交接门 20/21/22、model-routing 4、裁定被拒 30 都是这样给的）。旧文那句「非闸门工具不共用本语义、以各自头注释为准」只是**提示**，不是防护——撞码的代价最终落在读码的人身上，故本版按同一原则改码，不留第二处。
>
...
- `--top N` 于 v2.5.2-dsh.15 **真正实现**（此前只有头注释与 CHANGELOG 提到，代码里是死变量 `topMode=false`）：输出 `topByCacheRead` 排名（cacheRead、单会话成本估算、占总量百分比）。
  ...
  ```
- 未知参数与非法 `--top`（0/负数/非整数）现在一律 **exit 10** 并打印用法（旧版静默忽略 / 旧 v18.12.0 之前的 1 与「P1 内容失败」撞义、当前已收口到 10，见 §8 命名空间配额）。
