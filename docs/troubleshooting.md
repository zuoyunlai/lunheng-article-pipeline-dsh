...
> **命名空间配额（v18.12.0 登记，防下一个人随便挑数字）**：M 门族 `0/1/2/3` + 参数错 `10` + 内部错 `70`；**非 M 门语义一律另给码，且不得复用 0-3/10/70** —— 已有 `4`（model-routing 需人工决定）、`20/21/22`（交接门）、`30`（裁定被拒）、`40`（md2html 导出被拒）。下次新增非闸门退出码时从这里往后取，并在**同一次提交**里登记进 `EXIT_CONTRACT` + 本表。
>
