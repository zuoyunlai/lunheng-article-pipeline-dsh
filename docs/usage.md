...
论衡把一篇深度长文/论文的生产拆成 8 个角色、5 个阶段，由主控按协议派发子代理协作完成。
...
| 1 并行检索 | **T1 文献 ∥ T2 数据 ∥ T6 案例** 三方真并行（T6 任何量级必 spawn） | 文献卡 + 数据卡 + 案例卡 + 先行者清单 |
| 2 分析 | T3 分析员 | `analysis/分析大纲.md` |
| 2.5 大纲确认 | 主人过目（人在环） | — |
| 3 写作 | T4 写手 | `drafts/初稿-v1.md` |
| 3.6 批判 | T8 批判伙伴（C1-C5 反方攻击，轻量档可跳过） | `analysis/批判报告-vN.md` |
| 4 审计 | T5 审计员（G0-G13，只审不改） | `audits/审计报告-vN.md` |
| 4.2 修订 | 写手修订 ≤2 轮（独立写手执行） | `修订说明` + `v2/v3` |
| 5 终检 | 主控 M 门终检（M-Form/M-Exist/M-Integrity） | `final/定稿.md` + 图件 + 证据包 + 交付说明 |
...
├── analysis/分析大纲.md + 批判报告-vN.md（T8）
