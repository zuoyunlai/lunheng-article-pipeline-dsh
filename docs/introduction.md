...
终检前主控跑 **M 门**——LLM 兜底执行的机械化终检，**14 项**逐项机械判定（v2.5.2-dsh.0 统一为单文件算法）：

- **M-Form 10 项**：引用标注完整性 / 文末四节存在性 / 临时编号残留 / 角色元数据泄露 / 过程语言残留 / 信任级别标注完整性 / 文末节标题白名单纯净 / 三角验证覆盖率 / **图件闭环（[图N] ↔ final/图件/ ↔ 图上数字，v2.5.2-dsh.16 加） / **索引段完整性（索引 ↔ 正文条目，v2.5.2-dsh.17 加）**；
- **M-Exist 4 项**：文末四节双向 diff / 证据包文件完整性 / 信任级别一致性 / **审计条目闭环（P0/P1 任务书 ↔ 复核报告，v2.5.2-dsh.17 加）**；
- **M-Integrity 2 项**：T2.5 / T7.5 阶段闸门（主控 checkpoint 机械化核对）。
...
- **M 门 16 项终检（13 项机械判定 + 1 项主控人工门）**：形式合规 ≠ 存在性合规 ≠ 信任一致；M-Form-7 文末白名单（防操作员报告混入定稿）+ M-Form-8 三角验证机械化 + **M-Form-9 图件闭环（v2.5.2-dsh.16 加）**。
...
- **M 门 16 项全过 exit 0**，交付定稿 + 证据包 8 文件 + M-Gate-Report + 交付说明；
