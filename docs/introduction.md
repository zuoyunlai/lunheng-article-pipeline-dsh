...
> 适用：公众号深度长文、研究报告、学术论文、商业评论、行业分析——任何"要站得住脚"的长内容。当前版本 **v18.12.1**（DSH 独立版本）。
...
> **v2.3.0 角色编号重构 + v2.4.0 加 T9**：编号 = 流水线 Phase 顺序——T1-T3 检索 / T4-T5 加工 / T6-T9 防御。T8 终检由主控亲自完成（无独立角色卡），确保「最后一关」不外包；T9 同行评审是投稿前的「预演审稿人」，学术/行业分析模式默认开启。

六个阶段：**Phase 0 定题 → Phase 1 并行检索 → Phase 2 分析 → Phase 3 写作 → Phase 3.6 批判 → Phase 4 审计 → Phase 5 终检**，中间穿插 **T2.5 / T7.5 两道完整性门**（主控 checkpoint，不绕过交接直接派发）与 **4 个人在环节点**（定题 / 大纲 / 洞察补充 / 终稿）。
...
- **T9 同行评审（v2.4.0 加）**：投稿前的「预演审稿人」——6 维度评分（原创性/方法论/证据强度/论证结构/写作质量/引文规范，总分 30）→ accept/minor/major/reject；学术/行业分析模式默认开启。
- **G14 中文 AI 痕迹闸（v2.4.0 加）**：8 类检测维度深度扫描"AI 味"，0-2 类 Pass / 3-4 类 Warning / 5+ 类 Fail，与 T6 并行触发，LLM 推理判定零 exec。
- **期刊匹配助手（v2.5.0 加）**：基于 T9 评分 + 主题关键词，从 25 中文 CSSCI/北大核心 + 12 英文 SSCI 期刊库输出 Top 3 推荐期刊 + 综合匹配度；配套投稿就绪检查表。
...
- **npm**：`lunheng-article-pipeline@dsh`（dist-tag `dsh` = 最新 DSH 迭代版）；锁定具体版本可写 `@18.12.1`
