...
主控 Coordinator（定题、拆解、派发、终检、状态机）
  │
  ├─ T1 文献检索员 ── 文献卡 [Lxx]（A/B/C 可信度分级 + GB/T 7714 条目）
  ├─ T2 数据检索员 ── 数据卡 [Dxx]（🟢🟡🔴 时效评级 + 冲突口径并列）
  ├─ T6 案例检索员 ── 案例卡 [Cxx]（事件结构 + 多方立场 + ≥2 来源）
  ├─ T3 分析员     ── 分析大纲（论点-论据映射 + 反方论证规划）
  ├─ T4 写手       ── 初稿（引用/数据/案例全标编号）
  ├─ T5 审计员     ── 审计报告（G0-G11 全项检查，只审不改）
  └─ 主控终检      ── 定稿 + 证据包 + 交付说明
```
...
- **npm**：`lunheng-article-pipeline@2.1.8-dsh.1`（`npm i lunheng-article-pipeline@dsh`）
