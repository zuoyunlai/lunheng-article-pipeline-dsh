...
> 适用：公众号深度长文、研究报告、学术论文、商业评论、行业分析——任何"要站得住脚"的长内容。当前版本 **v18.18.10**（DSH 独立版本）。
...
- **期刊匹配助手（v2.5.0 加）**：基于 T9 评分 + 主题关键词，从期刊库（真源 = `references/_shared/期刊数据库.md` 表行数）输出 Top 3 推荐期刊 + 综合匹配度；配套投稿就绪检查表。**学术模式默认启用、主人可关；非学术默认关闭**（v18.16.0 与主控三岔口 #3 对齐）。
...
- **npm**：`lunheng-article-pipeline@dsh`（dist-tag `dsh` = 最新 DSH 迭代版）；锁定具体版本可写 `@18.18.10`
