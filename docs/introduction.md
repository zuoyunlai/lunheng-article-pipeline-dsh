...
- **M-Form 9 项**：引用标注完整性 / 文末四节存在性 / 临时编号残留 / 角色元数据泄露 / 过程语言残留 / 信任级别标注完整性 / 文末节标题白名单纯净 / 三角验证覆盖率 / **图件闭环（[图N] ↔ final/图件/ ↔ 图上数字，v2.5.2-dsh.16 加）**；
...
- **M 门 14 项终检（13 项机械判定 + 1 项主控人工门）**：形式合规 ≠ 存在性合规 ≠ 信任一致；M-Form-7 文末白名单（防操作员报告混入定稿）+ M-Form-8 三角验证机械化 + **M-Form-9 图件闭环（v2.5.2-dsh.16 加）**。
...
- **M 门 14 项全过 exit 0**，交付定稿 + 证据包 8 文件 + M-Gate-Report + 交付说明；
