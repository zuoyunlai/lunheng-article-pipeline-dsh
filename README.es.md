...
| Integración con DSH (instalación como bundle) | Tres herramientas **de solo lectura**: `lunheng_m_gate` (precomprobación mecánica de la puerta M), `lunheng_char_count` (recuento de caracteres chinos) y `lunheng_handoff_check` (verificación de forma / versión / pares / agents-log del informe de traspaso); si su sesión no las expone, invoque los mismos scripts con `pwsh` como antes (misma fuente de verdad). Comandos humanos `/lunheng-status` (lee `run/<proyecto>/status.md`) y `/lunheng-stats` (panel de telemetría entre proyectos; lanza `scripts/lunheng-stats.mjs` con `--json` como único argumento en lista blanca); ninguno genera mensajes de modelo. **Protección de escritura de los archivos de mecanismo**: un guard global rechaza las llamadas de herramientas tipo `write`/`edit` dirigidas al paquete de la skill, de modo que una sesión no puede reescribir en silencio las reglas de la propia pipeline. Límite declarado sin adornos: el guard solo ve **llamadas de herramientas**; `pwsh` y cualquier subproceso **no pasan por esta puerta**; la excepción del propietario es `LUNHENG_ALLOW_MECH_EDIT=1` (o `config: { allowMechanismEdit: true }`). El **Config** del plugin (interruptor de despliegue, en la fila de este plugin de su perfil) cubre `quiet`, `allowMechanismEdit`, `scriptTimeoutMs`, `scriptMaxOutputBytes` y `handoffLevel` — las dos primeras son equivalentes a las variables `LUNHENG_QUIET` / `LUNHENG_ALLOW_MECH_EDIT`, mientras que `scriptTimeoutMs` / `scriptMaxOutputBytes` / `handoffLevel` son **solo Config (sin variable de entorno)**; las cinco son revisables con el perfil; una **config inválida falla ruidosamente en la carga** en lugar de volver en silencio a los valores por defecto. |
...
├── skills/lunheng-commands/   # skill acompañante: 11 comandos de barra /lunheng-* (draft / resume / cite / audit / journal / ppt / history / rollback / status / stats / help)
...
```sh
git tag v18.20.2 && git push origin v18.20.2   # un tag por push (GitHub: >3 tags en un push no dispara workflow)
...
**Como directorio de skill simple** (sin instalación; el host vigila la raíz de skills y la refresca al cambiar — el `SKILL.md` del modo uno es una instantánea leída en el `apply` del entry, así que editar el contenido requiere recargar el plugin o abrir una sesión nueva):
...
| Elemento | Requisito |
|---|---|
| DSH (versión de producto) | `dsh` CLI disponible; las filas `- insert:` del bundle necesitan **DSH 5.5.0+ versión de producto** — nota: `dsh` se distribuye en dos líneas de versión paralelas, una **línea de producto** (semver, p.ej. `5.5.0`) y la **línea npm** `@deepseek-ai/dsh` (tags prerelease, p.ej. `0.1.7-rc.2`); **no son intercambiables** |
| `@deepseek-ai/dsh` (versión npm) | `peerDependencies` declara `>=0.1.2-rc.1 <0.2.0`; **CI solo prueba `0.1.7-rc.2`** (tag prerelease fijado en `ci.yml`); versiones `0.1.x` anteriores pueden o no funcionar — el límite inferior es **declaración**, no verificación |
...
Puertas locales (**solo fuentes del repositorio** — el paquete npm **no incluye** `scripts/` ni `tests/`, por lo que los comandos siguientes no se pueden ejecutar en una instalación desde el paquete; dentro del paquete puede ejecutar el conjunto equivalente sobre el tarball recién descargado con `pack-smoke`): `node skills/lunheng-article-pipeline/scripts/consistency-check.mjs`, `node scripts/plugin-surface-check.mjs`, `node scripts/repo-hygiene-check.mjs`, `node scripts/pack-smoke.mjs`, `node --test "tests/**/*.test.mjs"`.
