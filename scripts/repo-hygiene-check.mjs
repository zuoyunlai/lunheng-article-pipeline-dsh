...
const DOC_BUDGET = {
  ...
  'skills/lunheng-article-pipeline/AGENTS.md': [21504, 16384, '操作手册：技能目录内自动生效的指令——**v18.20.3 首次显式瘦身 21463→20552 B（余量 41→952 B；上限维持 21 KB）**：按「一事实两处 / 注解聚合」聚合 6 处——① 收口批「差集+反向核验」的完整过程与两次自证**已在 `references/maintainers.md` §七**（1005→约 250 B，改结论 + 指针）；② 「仓库级打包面检查」清单与 §开发参考资料重复（670→约 260 B）；③ 闸门退出码的 v18.12.0 L-67 历史细节收敛为「判据一句话」（判据保留）；④ 子代理失败三段式的版本注解（教训留 git log）；⑤ status/agents-log 历史括注；⑥ `dsh-plugin-dev check` 清单与 §开发参考资料重复 → 改指针。**长期目标 16 KB 不变**（当前 20.1 KB——后续可继续把维护者向内容外移到 `references/maintainers.md`）。'],
