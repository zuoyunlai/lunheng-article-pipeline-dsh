...
const DOC_BUDGET = {
  ...
  'skills/lunheng-article-pipeline/references/agents/05-写作-writer.md': [40960, 36864, 'T5 写手卡——v18.2.5 显式抬升 36→38 KB：补「段级 diff 字数口径与 count-chars 同源（两侧汉字计数）」+「图位编号 = 正文出现顺序，禁照抄大纲编号」两条硬约束；**v18.2.8 显式抬升 38→39 KB（主人授权「G14 早闸去掉」）**：F 自检 7 项 → 7+1 项，新增「G14 v1 自检」（早闸删除后由写手在写作阶段承担第 1 层检测）；**v18.2.9 显式抬升 39→40 KB（G 体系机械下沉）**：写作规范新增「删优于改（Prefer CUT over REWRITE）」一条（借鉴 Writing Guard）'],
  ...
  'skills/lunheng-article-pipeline/references/_shared/规范-机械门对照表.md': [23552, 16384, '规范条文 ↔ 机械门 ID 对照表（改机制前须同步的一览）——v18.2.2 新增登记；v18.2.4 显式抬升 16→17 KB：补 3 行断链；v18.2.5 显式抬升 17→22 KB：补 7 行（门补面与同步本表是同一件事的两半）；v18.2.9 显式抬升 22→23 KB（G 体系机械下沉）：补「因果强度守恒（apply-diff causal_upgrades）+ 裸断言段（M-Form-8 子项）」两行勾稽'],
}
