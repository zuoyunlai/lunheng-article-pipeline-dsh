...
const DOC_BUDGET = {
  ...
  'skills/lunheng-article-pipeline/references/agents/05-写作-writer.md': [46080, 36864, 'T5 写手卡——v18.2.5 显式抬升 36→38 KB：补「段级 diff 字数口径与 count-chars 同源（两侧汉字计数）」+「图位编号 = 正文出现顺序，禁照抄大纲编号」两条硬约束；**v18.2.8 显式抬升 38→39 KB（主人授权「G14 早闸去掉」）**：F 自检 7 项 → 7+1 项，新增「G14 v1 自检」（早闸删除后由写手在写作阶段承担第 1 层检测）；**v18.3.0 显式抬升 39→40 KB（G 体系机械下沉）**：写作规范新增「删优于改（Prefer CUT over REWRITE）」一条（借鉴 Writing Guard）；**v18.7.1 显式抬升 40→45 KB（共锁反哺：G14 实测铁律 / 图位编号 / 幽灵引用 / 双口径字数 / 元数据红线 / 修订说明等量覆盖 四组补丁）**'],
  ...
  'skills/lunheng-article-pipeline/references/agents/07-审计-auditor.md': [24576, 22528, 'T7 审计卡——v18.5.1 显式抬升 22→23 KB：修订任务书新增「违反规范」列 + 「怎么改」列处置动作动词前缀（补/删/改/降级），借鉴 writing-guard rule/action 分离（反哺报告-writing-guard借鉴-v1 动议一）；**v18.7.1 显式抬升 23→24 KB（共锁反哺：铁律加 T6 交叉验证）**'],
