...
const DOC_BUDGET = {
  ...
  'skills/lunheng-article-pipeline/references/glossary.md': [35840, 32768, '术语表（含 §十二 刻意偏离，改机制前必读）——v18.5.1 显式抬升 32→34 KB（ai-content-farm-retractions 反哺：§12.1 正文字数单一真源 + §12.3 五节 vs 国标仲裁）；**v18.7.0 显式抬升 34→35 KB（借鉴 Ai4Scholar 落地）**：§核心角色新增 T3.5 文献补标注角色卡（可选阶段，auto_cite 预扫描）；§十一 关键术语新增「引用匹配度」（与 G15 审计项 + M-Exist-6 联动）。依据 v18.1.0「词预算预冲」原则显式抬升上限；目标（长期）维持 32 KB。'],
  ...
  'skills/lunheng-article-pipeline/references/_shared/audit-checklist-quickref.md': [12800, 10240, '审计必查项快速参考——v18.7.0 首次登记（借鉴 Ai4Scholar v2.9.4 落地）：§必查项尾部新增 G15 引用匹配度（任务简报 §引用数量与质量控制 字段核验：引用数区间 / 优先同刊比例 / IF 门槛 / 卷期页码完整率 / auto_cite 替换率 + 修订任务书条目联动）。依据 v18.1.0「词预算预冲」原则显式登记上限；目标（长期）维持 10 KB。'],
  'skills/lunheng-article-pipeline/references/templates/任务简报-template.md': [24576, 19456, '任务简报模板（full 版）——v18.2.2 显式抬升 19→21 KB：① 补「字数判定层级强制显式勾选 + 与 G5 硬阈的关系澄清」；② 补「GB/T 7714 只约束著录格式、不约束标签形态」正误形态对照（第二处抬升，20480→21504）——**v18.5.1 显式抬升 21→22 KB（ai-content-farm-retractions 反哺：「需找数据点 vs 需找案例」标签口径说明）**；**v18.7.0 显式抬升 22→24 KB（借鉴 Ai4Scholar 落地 4 份反哺报告整合）**：§引用格式段后追加 §引用数量与质量控制 字段（目标引用数 / 目标期刊 / IF 门槛 / JCR 分区 / 优先同刊 / auto_cite 替换预算）+ §卷期页码完整性 段；§v2.5.0 可选项追加 3 个新勾选项（APA 优先输出 + T3.5 + /lunheng）。依据 v18.1.0「词预算预冲」原则显式抬升上限；目标（长期）维持 19 KB。'],
