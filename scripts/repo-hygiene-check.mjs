...
const DOC_BUDGET = {
  ...
  'skills/lunheng-article-pipeline/references/deliverables.md': [18432, 16384, '交付边界 + F1-F9 + 闸门（按需加载）——v18.2.1 显式抬升 16→17 KB：补「文末编号必须沿用素材卡真编号」硬要求；**v18.12.0 显式抬升 17→18 KB（2026-09-25 全量审计响应 L-17）**：「定稿文末白名单」由“只允许 5 节”改写为「必需 5 节（M-Form-2 存在性）+ 可选 4 学术声明（仅 M-Form-7 成员/顺序）」两层表 + 机检真源指针——旧表述与 v18.10.0 起写手卡要求的「文末九节」互斥，学术稿照规范写必被 M-Form-7 判 P0 且该门不可兜底（实测 exit=2）'],
  ...
  'skills/lunheng-article-pipeline/references/_shared/规范-机械门对照表.md': [26624, 16384, '规范条文 ↔ 机械门 ID 对照表（改机制前须同步的一览）——v18.2.2 新增登记；v18.2.4 显式抬升 16→17 KB：补 3 行断链；v18.2.5 显式抬升 17→22 KB：补 7 行（门补面与同步本表是同一件事的两半）；v18.3.0 显式抬升 22→23 KB（G 体系机械下沉）：补「因果强度守恒（apply-diff causal_upgrades）+ 裸断言段（M-Form-8 子项）」两行勾稽；**v18.7.1 显式抬升 23→24 KB（共锁反哺：§11 标题唯一性 + 闸门记录裸竖线 两行假阳性登记）**；**v18.12.0 显式抬升 24→26 KB（2026-09-25 全量审计响应 L-17 + L-20）**：新增三行勾稽——① 文末九节白名单（「先有规范、无门」→ 本次补齐 M-Form-7 九节断言）；② 四声明档位勾选（**无门**，原「T7 跑 M-Form-12 子门」系假绿，已改判「T7 人工核验」）；③ `methodology-check.mjs` 的 M-Form-12 / M-Exist-11 / M-Exist-12 命名空间撞号登记（`M-Exist-11` 在本表指「局限性门」、在该脚本指「统计-数据匹配」）'],
}
