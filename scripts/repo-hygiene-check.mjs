...
const DOC_BUDGET = {
  ...
  'skills/lunheng-article-pipeline/references/glossary.md': [35840, 32768, '术语表（含 §十二 刻意偏离，改机制前必读）——v18.5.1 显式抬升 32→34 KB（ai-content-farm-retractions 反哺：§12.1 正文字数单一真源 + §12.3 五节 vs 国标仲裁）；**v18.7.1 显式抬升 34→35 KB（借鉴 Ai4Scholar 落地）**：§核心角色新增 T3.5 文献补标注角色卡（可选阶段，auto_cite 预扫描）；§十一 关键术语新增「引用匹配度」（与 G15 审计项 + M-Exist-6 联动）。依据 v18.1.0「词预算预冲」原则显式抬升上限；目标（长期）维持 32 KB。'],
  'skills/lunheng-article-pipeline/references/agents/05-写作-writer.md': [53248, 36864, 'T5 写手卡——v18.2.5 显式抬升 36→38 KB：补「段级 diff 字数口径与 count-chars 同源（两侧汉字计数）」+「图位编号 = 正文出现顺序，禁照抄大纲编号」两条硬约束；**v18.2.8 显式抬升 38→39 KB（主人授权「G14 早闸去掉」）**：F 自检 7 项 → 7+1 项，新增「G14 v1 自检」（早闸删除后由写手在写作阶段承担第 1 层检测）；**v18.3.0 显式抬升 39→40 KB（G 体系机械下沉）**：写作规范新增「删优于改（Prefer CUT over REWRITE）」一条（借鉴 Writing Guard）；**v18.7.1 显式抬升 40→45 KB（共锁反哺：G14 实测铁律 / 图位编号 / 幽灵引用 / 双口径字数 / 元数据红线 / 修订说明等量覆盖 四组补丁）**；**v18.9.0 显式抬升 45→52 KB（数字社交-关系重构项目实战反哺 v18.8.x + v18.9.0 两轮承重）**：①字数估算 +20% buffer 段（治 LLM 估算 1.44x 偏差，v18.8.x 反哺）——含边界 / 实战教训 3 轮 / 机检留痕 3 项；②「🚫 元数据泄露词表」段（v18.9.0 反哺，与 M-Form-4/5 黑名单同源）——含词表 28 行 / 判定 3 类 / 文末五节白名单 1 类 / AI 使用声明 1 处豁免。两条均为实战反哺的承重内容（不挪 references/），按「同一次提交显式抬升」原则同步抬上限；目标（长期）维持 36 KB（v18.9.1 起考虑瘦身）。'],
  ...
  'skills/lunheng-article-pipeline/references/_shared/规范-机械门对照表.md': [24576, 16384, '规范条文 ↔ 机械门 ID 对照表（改机制前须同步的一览）——v18.2.2 新增登记；v18.2.4 显式抬升 16→17 KB：补 3 行断链；v18.2.5 显式抬升 17→22 KB：补 7 行（门补面与同步本表是同一件事的两半）；v18.3.0 显式抬升 22→23 KB（G 体系机械下沉）：补「因果强度守恒（apply-diff causal_upgrades）+ 裸断言段（M-Form-8 子项）」两行勾稽；**v18.7.1 显式抬升 23→24 KB（共锁反哺：§11 标题唯一性 + 闸门记录裸竖线 两行假阳性登记）**'],
}
