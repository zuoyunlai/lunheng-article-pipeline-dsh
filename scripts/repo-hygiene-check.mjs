...
const DOC_BUDGET = {
  ...
  'skills/lunheng-article-pipeline/references/pipeline-readme.md': [79872, 61440, '流水线全景 + 派发话术（T0 启动必读）——v18.2.2 显式抬升 60→64 KB：补「段级 diff 字数回测硬约束」与「T5/T6/T9 派发用 M 门速查」；v18.2.5 显式抬升 64→65 KB：补「段级 diff 字数口径硬约束（纯汉字 = count-chars 同源）」与「审计视图刷新落盘留痕（status.md 可核对项）」；**v18.2.7 显式抬升 65→74 KB（主人授权修订）**：依 2026-09-20 全流程实战反哺，一处补 6 条硬约束——① 派发最小集补第 7 条「文末五节条目标签用论衡编号」（真源 `机检硬格式.md` §五 v18.2.2 已有该条，但最小集一直漏同步，实战第三次踩到：T5 四版全写 `[1]`-`[11]`，T7 三轮未抓出，直到 T8 才爆 P0）；② 段级 diff 加「预算闸门前置」（清单预估 16,987 → 实测 18,071，超限 1,271 字 → 被迫削 3 轮）；③ 加「素材加载清单刷新责任在段级 diff 模式下归主控」（与「T5 每轮覆盖写」机制直接冲突）；④ 加「清单须机器可读、优先用 apply-diff.mjs」（实战主控为此做约 52 处手工 edit）；⑤ T7 修订任务书加「验收方式」列（解「字面验收 vs 字数约束」死结）；⑥ T7 预检从 5 项扩为**全量 22 项 M 门** + 缺陷冻结机制（3 个零判断力 P0 漏到 T8、A 轨 2 轮用满仅 29% 完全关闭）；**v18.2.8 显式抬升 74→75 KB（主人授权「G14 早闸去掉」）**：流水线全景的 G14 行与派发话术段改为「三层防御、仅一次 spawn」。**已先做两轮瘦身**（删除依据移至 `gates/14` 单点持有），瘦身后仍超 150 B；剩余为 Phase 序列与派发话术本身，属 T0 启动必读的承重内容；**v18.5.1 显式抬升 75→78 KB（ai-content-farm-retractions 反哺：派发前 preflight 反注 + T6/T9 意见强制机械证据硬约束）**'],
  'skills/lunheng-article-pipeline/references/_shared/M-Gate-Algorithm.md': [88064, 79872, 'M 门伪代码（仅 T7/T8 读）——v18.2.2 显式抬升 78→84 KB：补 6 处 v18.2.2 判定语义修订注记（M-Form-4/8/11 + M-Exist-2/10 + M-Integrity-1）；**v18.3.1 显式抬升 84→86 KB（第三方审计 B3）**：新增「阈值总表」生成块（24 个阈值键，由 m-gate-check.mjs `THRESHOLDS` 单向生成 + consistency-check ㉓ 逐键核对，消灭阈值数字与正文伪代码的双维护）'],
  'skills/lunheng-article-pipeline/references/agents/00-主控-扩展职责.md': [54272, 50176, 'T0 实操手册（全库第二，闸门公共动作在此）——v18.2.2 显式抬升 49→52 KB：补「修订回环提前 Ack 判据」四类归因决策表；v18.2.9 显式抬升 52→53 KB（第三方审计 A12）：补「T8 人工裁定双签」（M-Form-8 / M-Integrity-2 亲裁定须独立复核，守住独立角色互为镜像）'],
  ...
  'skills/lunheng-article-pipeline/references/agents/05-写作-writer.md': [40960, 36864, 'T5 写手卡——v18.2.5 显式抬升 36→38 KB：补「段级 diff 字数口径与 count-chars 同源（两侧汉字计数）」+「图位编号 = 正文出现顺序，禁照抄大纲编号」两条硬约束；**v18.2.8 显式抬升 38→39 KB（主人授权「G14 早闸去掉」）**：F 自检 7 项 → 7+1 项，新增「G14 v1 自检」（早闸删除后由写手在写作阶段承担第 1 层检测）；**v18.3.0 显式抬升 39→40 KB（G 体系机械下沉）**：写作规范新增「删优于改（Prefer CUT over REWRITE）」一条（借鉴 Writing Guard）'],
  ...
  'skills/lunheng-article-pipeline/references/agents/07-审计-auditor.md': [23552, 22528, 'T7 审计卡——v18.5.1 显式抬升 22→23 KB：修订任务书新增「违反规范」列 + 「怎么改」列处置动作动词前缀（补/删/改/降级），借鉴 writing-guard rule/action 分离（反哺报告-writing-guard借鉴-v1 动议一）'],
  ...
  'skills/lunheng-article-pipeline/references/agents/09-审稿-peer-reviewer.md': [19456, 13312, 'T9 审稿卡——v18.2.1 显式抬升 13→14 KB：补 M-Exist-6 六维机检契约警示（防派发时改写维度名）；**v18.2.7 显式抬升 14→18 KB（主人授权修订）**：依 2026-09-20 全流程实战反哺，补三处——① **修本卡内部矛盾**：§when 写「T9 只能在 T7 审计后」而 §exclusions 写「❌ T7 审计后（已晚）」，同一张卡两处直接冲突（v2.4.0 引入 G14 时点漂移时的遗留），现删除错误项并注明正确时点；② 新増铁律「我是参谋层，不是交付闸门」——判定词 accept/minor/major/reject 借自真实同行评审、天然带程序效力暗示，若机制不写明「T9 不阻塞交付」，主控在 A 轨用满时读到「major revision」可能被迫多跑一轮；③ 格式骨架补「综合匹配度」**表的字面样例**（此前只有文字说明 → T9 按卡产出自然语言段落 → M-Exist-6 判 P1）；**v18.5.1 显式抬升 18→19 KB（writing-guard 借鉴反哺 v1，动议二+动议一）**：扩写清单扩为「字数+文体偏差」双维度 + 建议段 rule/action 分离（违反规范 + 处置动作）'],
  ...
  'skills/lunheng-article-pipeline/references/_shared/规范-机械门对照表.md': [23552, 16384, '规范条文 ↔ 机械门 ID 对照表（改机制前须同步的一览）——v18.2.2 新增登记；v18.2.4 显式抬升 16→17 KB：补 3 行断链；v18.2.5 显式抬升 17→22 KB：补 7 行（门补面与同步本表是同一件事的两半）；v18.3.0 显式抬升 22→23 KB（G 体系机械下沉）：补「因果强度守恒（apply-diff causal_upgrades）+ 裸断言段（M-Form-8 子项）」两行勾稽'],
}
