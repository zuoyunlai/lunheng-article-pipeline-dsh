...
> **DSH 原生插件（v18.18.13）**：本仓库为 DSH bundle 技能包（`package.json` 的 `main` → `lib/index.js` 注册本技能 + `dsh.bundle.patch` → `cordis.patch.yml` 叠加三档 subagent 工具）；结构性差异见 `SKILL.md` 的「🔧 DSH 环境说明」章节。
...
### 九角色流水线（v2.3.0 编号重构；v2.5.2-dsh.8 定案：T1-T9 九角色互不可替代）
...
审计员与写手分离，全项检查：G0 覆盖度 · G0.5 视角一致性 · G1 引用核验 · G2 数据溯源 · G2.5 案例核验 · G3 逻辑 · G4 格式 · G4-2 文末四节 · G5 规范 · G6 论据自标 · G7 原创性 · G8 成品度 · G9 时序 · G10 一致性 · G11 时效 · G12 信任级别 · G13 AI 披露 · **G14 中文 AI 痕迹闸**（v2.4.0 加：8 类检测维度，0-2 类 Pass / 3-4 类 Warning / 5+ 类 Fail；v18.2.8 起三层防御——T5 v1 自检 → 修订轮收尾自查 → 终闸与 T9 并行 spawn）。终检前主控跑 **M 门**（M-Form 11 / M-Exist 10 / M-Integrity 2，LLM 兜底执行，产出 `final/M-Gate-Report.json`，exit 0 才交付）。
