...
> **DSH 适配版（v2.5.2-dsh.8）**：本仓库为 DSH bundle 插件（技能包）；工具映射与结构性差异见 `SKILL.md` 的「🔧 DSH 适配说明」章节。

把一篇深度文章 / 论文的生产拆成 **9 个独立角色 T1-T9 + 6 个阶段**（v2.5.2-dsh.8 定案：T1-T9 互不可替代；Phase 1 为 T1 文献 ∥ T2 数据 ∥ T3 案例 三检索员三方真并行互不干涉，T3 任何量级必 spawn 含 0 条空卡协议；T6 批判伙伴 + T7 审计 + **T8 终检独立角色由主控 T0 亲执行** + T9 同行评审可选默认选中学术必选，v2.3.0 角色编号重构 + v2.4.0 加 T9），用 DSH 的 `subagent` 子代理编排，产出有**证据底座、反方论证、独立审计、人工核验节点**的交付物。
...
主控 Coordinator（定题、拆解、派发、阶段闸门、终检、状态机）
  ...
  └─ T8 终检（独立角色，主控 T0 亲执行）── M 门（M-Form 8 / M-Exist 3 / M-Integrity 2）+ 定稿 + 证据包
```
...
Phase 4.5 审稿      T9 同行评审（可选，默认选中，学术论文必选）→ 审稿报告 + 期刊匹配 Top 3；G14 中文 AI 痕迹闸（与 T6 并行）
...
└── skills/lunheng-article-pipeline/         # 技能本体
    ...
    └── references/
        ├── agents/                          # **9 个独立角色卡 T1-T9**（01-09；T8 终检由主控 T0 亲执行；00 主控 = T0 + T8 双重身份）
        ...
        │   ├── 07-审计-auditor.md
        │   ├── 08-终检-finalizer.md          # T8 终检（独立角色，主控 T0 亲执行，v2.5.2-dsh.8 新增独立卡）
