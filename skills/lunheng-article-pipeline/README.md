...
Phase 4.5 审稿      T9 同行评审（可选，学术/行业分析默认开）→ 审稿报告 + 期刊匹配 Top 3；G14 中文 AI 痕迹闸（与 T6 并行）
...
└── skills/lunheng-article-pipeline/         # 技能本体
    ...
    └── references/
        ├── agents/                          # **9 张角色卡**（00 主控 + 00 扩展 + 01-07 + 09 审稿，T8 终检=主控亲完成）
        ...
        ├── memory/                          # 教训库（lessons.md，论衡特定教训，每单实战沉淀）
```
...
| 甲醛白菜事件深度文（2026-08） | ~4200 字，12 文献 + 29 数据 + 4 案例 | v2.5.2-dsh.0 同步前实测：M 门 13 项 exit 0 + 反哺 6 规则 merge + 教训 #142-#144 |
