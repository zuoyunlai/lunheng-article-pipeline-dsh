...
> **DSH 原生插件（v2.5.2-dsh.16）**：本仓库为 DSH bundle 技能包；结构性差异见 `SKILL.md` 的「🔧 DSH 环境说明」章节。
...
主控 Coordinator（定题、拆解、派发、阶段闸门、终检、状态机）
  ...
  └─ T8 终检（独立角色，主控 T0 亲执行）── M 门（M-Form 11 / M-Exist 7 / M-Integrity 2）+ 定稿 + 证据包
```
...
└── skills/lunheng-article-pipeline/         # 技能本体
    ...
    └── references/
        ...
        │   ├── M-Gate-Algorithm.md          # M 门算法（M-Form 11 / M-Exist 7 / M-Integrity 2）
