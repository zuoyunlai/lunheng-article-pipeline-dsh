...
> **DSH 原生插件（v18.2.8）**：本仓库为 DSH bundle 技能包（`package.json` 的 `main` → `lib/index.js` 注册本技能 + `dsh.bundle.patch` → `cordis.patch.yml` 叠加三档 subagent 工具）；结构性差异见 `SKILL.md` 的「🔧 DSH 环境说明」章节。
...
主控 Coordinator（定题、拆解、派发、阶段闸门、终检、状态机）
  ...
  └─ T8 终检（独立角色，主控 T0 亲执行）── M 门（M-Form 11 / M-Exist 10 / M-Integrity 2）+ 定稿 + 证据包
```
...
| 甲醛白菜事件深度文（2026-08） | ~4200 字，12 文献 + 29 数据 + 4 案例 | v2.5.2-dsh.0 同步前实测：M 门 exit 0（当时为旧口径；现行真源=机械 22 + 人工 1 = 23） + 反哺 6 规则 merge + 教训 #142-#144 |
