...
3. 读 `MEMORY.md` 了解主人偏好（输出风格 / 沟通方式 / 重要教训）——若技能包/项目目录尚无该文件，说明主人尚未沉淀偏好，跳过即可
4. 读 `memory/YYYY-MM-DD.md`（今天+昨天）看主人最近关注主题
5. 项目目录固定 `run/<项目名>/`，路径映射见本文件「项目目录结构」段
...
| **3000-5000 字** | 推荐全量流水线 | 标准 8 角色，T6 任何量级必 spawn，T8 视论证强度可选 |
| **2000-3000 字** | 可走轻量档 | T1/T2 必跑，T6 0 条空卡协议，T8 可跳过，T3 大纲可省（按模板出） |
...
Phase 4 审计        T5 审计员 → audits/审计报告-vN.md（G0-G13 全项：覆盖度/引用核验/数据溯源/逻辑/格式/规范/案例核验/论据自标/原创性/成品度/时序/一致性/时效/信任级别/AI 披露）
...
├── drafts/初稿-vN.md    # T4 产出 + 修订稿 v2/v3（修订按「先 cp 新版本再改」SOP，vN 保持原始）+ 修订说明
...
1. **证据底座先行 + 三角验证**：任何论点必须能映射到三类证据——文献卡[Lxx]（学术依据）+ 数据卡[Dxx]（量级数据）+ 案例卡[Cxx]（具体事件，涉企业行为/事件者必须配），至少两项齐全、涉事件者必须有案例卡；检索不到就标缺口，严禁编造
...
**审计必查项**：G0-G13 审计清单（G0 覆盖度 / G0.5 视角 / G1 引用核验 / G2 数据溯源 / G2.5 案例核验 / G3 逻辑 / G4 格式 / G4-2 文末四节 / G5 规范 / G6 论据自标 / G7 原创性 / G8 成品度 / G9 时序 / G10 一致性 / G11 时效 / G12 信任级别 / G13 AI 披露）的逐条详解 + M 门算法见 [`references/_shared/audit-checklist-quickref.md`](references/_shared/audit-checklist-quickref.md) 与 [`references/agents/05-审计-auditor.md`](references/agents/05-审计-auditor.md)。SKILL.md 不重复维护，避免文档漂移（教训 #60）。

**派发话术锚点速查**（主控读 pipeline-readme.md 后定位用）：
- T1 文献检索员 → `## 派发话术` →「文献检索员（并行①）」
- T2 数据检索员 →「数据检索员（并行②）」
- T6 案例检索员 →「案例检索员（v2.1.8 强化…）」
- T3 分析员 →「分析员」
- T4 写手 →「写手」
- T8 批判伙伴 →「批判伙伴（v2.2.2 新增…）」
- T5 审计员 →「审计员」

**审计锚点速查**：
- G0-G13 逐条 → `references/_shared/audit-checklist-quickref.md`
- G 体系 + M 门 + F 模式 → `references/agents/05-审计-auditor.md`「M 门算法 + 必查项 + F 失败模式」
