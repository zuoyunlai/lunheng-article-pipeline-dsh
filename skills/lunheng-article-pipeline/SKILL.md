...
## 🔧 DSH 适配说明（v2.3.7-dsh.7 — 从 OpenClaw v2.3.7 移植到 DeepSeek Harness）
...
5. **模型分工（能力定位，v2.3.7-dsh.6 通用化，不写死具体模型名）**：检索用便宜快模型，分析/写作用推理强模型，审计用顶配模型，主控负责判断路由——具体模型由 DSH `settings.yaml` / 分档预设 `LUNHENG_*_PROVIDER`+`_MODEL` 决定（任何 dsh 可用的模型都能承担对应角色）
