...
**全程硬约束**
7. **Phase 0 决策必须落盘 `阶段确认-Phase0.md`**（v18.0.0）：四门（0 / 2.5 / 3.5 / 5）通用同一模板；Phase 0 的「4 选 1 外发同意 + 项目名 + 篇幅 + 引用格式」写入 §6 可回填表（**v18.12.0，L-41**：旧文本还列了「学派」，但全库**没有任何采集点**——`任务简报-template-lite` 与 `主人确认-template` 的 Phase 0 字段都没有它；幽灵字段已删，若主人要「学派」请先加进模板再回填此处）。**缺此文件 → 交付说明 §11 只能如实标「未留痕」**（实战本项目即如此）。
...
- 版本：v18.12.1｜角色：T0 主控（= T8 终检执行者）＋ T1 文献 / T2 数据 / T3 案例 / T4 分析 / T5 写作 / T6 批判 / T7 审计 / T8 终检（主控亲执行）/ T9 审稿（默认选中，学术必选）
