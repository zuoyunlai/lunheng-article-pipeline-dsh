...
| **T1 文献检索** | 已发布学术文献检索 | Phase 1（与 T2/T3 并行） |
...
> ⚠️ **修订回环语义（v2.3.7-dsh.8 定案，教训 #67 + #117）**：≤2 轮硬约束指**审计阶段独立预算**——审计发现 P0/P1 打回后，写手修订 → 审计复核，最多 2 轮；第 3 轮触发 → Acknowledged Limitations 模式。前序的 v1→v2（Phase 3.5 主人洞察）+ v2→v3（Phase 3.6 批判反馈）**不计入审计修订预算**（它们是人机协作的独立轮次，不是审计打回的修订轮）。
