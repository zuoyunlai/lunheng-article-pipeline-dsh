...
- **执行模型**（v2.2.12 澄清）：
  ...
  - ❌ **不执行 shell**：算法中的 `grep`/`diff`/`sha256sum`/`wc` 等命令仅供人类手动验证
...
- **不执行 shell**：算法文档中的 `grep`/`diff`/`sha256sum`/`wc` 等命令仅供人类主人手动验证（正典 v2.5.2 净化版中的「（检查）」占位符同样按此处理）
- **纯推理流水线**：论衡是 LLM 推理 + 文件读写 + Web 检索，主流程不依赖 shell 执行
