...
- **T8 终检职责**：M 门 16 项复核 + 证据包自动生成 + AI 使用声明 + 交付物完整性 + 引用闭环 → `final/定稿.md` + `final/证据包/` + `final/交付说明.md`（详见 [`agents/08-终检-finalizer.md`](agents/08-终检-finalizer.md)）
...
- **职责**：M 门 16 项复核 + 终检必查 15 项 + 证据包自动生成 + 交付物完整性 + AI 使用声明填实
...
- **执行模型**（v2.2.12 澄清，v2.5.2-dsh.4 审计修订）：
  ...
  - ✅ **白名单脚本机械判定**：M-Form 1-10 + M-Exist 1-4 + M-Integrity-1（共 15 项）走 `scripts/m-gate-check.mjs`（零 LLM，可复现 exit code）
  - ❌ **不执行任意 shell**：算法中的 `grep`/`diff`/`sha256sum`/`wc` 等命令示例仅供人类手动复核，agent 优先用白名单脚本/read 推理
- **覆盖范围**：13 项检查（M-Form 10 项 + M-Exist 4 项 + M-Integrity 2 项）
...
### ℹ️ 关键澄清
- **M 门算法**：主控 LLM 通过 `read` 读取算法文档，按伪代码**推理判定**；机械判定项（M-Form 1-10 + M-Exist 1-4 + M-Integrity-1）走 `scripts/m-gate-check.mjs`
