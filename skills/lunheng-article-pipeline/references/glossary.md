...
- **规划**：todo_write
- **命令执行**：pwsh（Windows）/ bash（Linux）——论衡主流程默认不执行任意 shell（LLM 推理判定），白名单例外：随包 7 个 `scripts/*.mjs` + 有限验证命令（ls/stat/wc/cp/diff/Get-FileHash 等，主控执行、受沙箱约束），需其他命令时经主人同意（v2.5.2-dsh.5 审计修订：如实声明，非「零 exec」；v2.5.2-dsh.8 扩为 7 个含 build-evidence-bundle）
