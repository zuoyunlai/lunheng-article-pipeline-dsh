...
## 职责
- **M 门 16 项全复核**：读 `_shared/M-Gate-Algorithm.md`，机械项先跑 `scripts/m-gate-check.mjs`（**15 项**：M-Form 1-10 + M-Exist 1-4 + M-Integrity-1 佐证），LLM 判项（M-Form-2/4/6/8 + M-Exist-1/3 + M-Integrity-1/2）逐项复核，产出 `final/M-Gate-Report.json`，**exit 0 才返回**
...
5. **一键终检脚本 final-check.mjs（v2.5.2-dsh.8 新增，关键优化）**：Phase 5 终检时**直接跑 `node scripts/final-check.mjs <run/项目名>`**，自动串联：
   ...
   - `m-gate-check.mjs <定稿.md> <证据包>`（**M 门 15 项机械化**，含 v2.5.2-dsh.16 新增的 M-Form-9 图件闭环；M 门总 14 项另含 M-Integrity-2 主控人工门）
...
### 3. 审计视图 + 素材卡全集（深度摘要）
`audits/审计视图-v0.md` 含：**视图源 + 阶段**（定稿/草稿快照/无正文源——v2.5.2-dsh.15 起写入视图头）+ 正文结构与纯汉字数 + 素材卡（L/D/C）数量 + 信任级别分布 + M 门 16 项状态 + 引用闭环 + 报告存在性 + 待确认项。生成方式：`node scripts/build-evidence-bundle.mjs <项目> --deep-summary`（含 L/D/C 前 30 条标题 + 信任级别 + DOI；T8 一次看完全部素材卡，无需逐文件 grep）。Phase 5 若要显式锁定定稿源：`--source final/定稿.md`。
