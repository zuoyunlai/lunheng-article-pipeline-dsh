...
### 3. 审计视图 + 素材卡全集（深度摘要）
`audits/审计视图-v0.md` 含：**视图源 + 阶段**（定稿/草稿快照/无正文源——v2.5.2-dsh.15 起写入视图头）+ 正文结构与纯汉字数 + 素材卡（L/D/C）数量 + 信任级别分布 + M 门 13 项状态 + 引用闭环 + 报告存在性 + 待确认项。生成方式：`node scripts/build-evidence-bundle.mjs <项目> --deep-summary`（含 L/D/C 前 30 条标题 + 信任级别 + DOI；T8 一次看完全部素材卡，无需逐文件 grep）。Phase 5 若要显式锁定定稿源：`--source final/定稿.md`。
