...
### M-Form-2: 文末白名单节存在性（v2.5.2-dsh.5 修订：与 M-Form-7 统一为 5 节 + v18.7.1 卷期页码扩展）

> **v18.7.1 扩展（借鉴 Ai4Scholar v2.9.4）**：启用 APA 优先输出时（任务简报 §v2.5.0 可选项 ✓ APA 优先输出），
...
### M-Form-10: 索引段完整性（v2.5.2-dsh.17 新增 + v18.7.1 T3.5 auto_cite-补充.md 扩展）

> **v18.7.1 扩展（借鉴 Ai4Scholar v2.9.1）**：T3.5 auto_cite 预标注阶段（主人 Phase 0 显式开启）产出
...
### M-Exist-6: 审稿报告与期刊匹配（v2.5.2-dsh.17 新增 + v18.7.1 G15 引用匹配度扩展）

> **v18.7.1 扩展（借鉴 Ai4Scholar v2.9.4）**：任务简报 §引用数量与质量控制 字段启用时，
> 审稿报告 §引用匹配度 段须满足：
...
> **定位口径（v18.7.1 登记）**：本项以**标题开头**含 `§11`（`^#{2,4}\s*§11`）且含「写手版 / 精简段 / 精简」的标题为锚（优先匹配），找不到时回退到宽松匹配（兼容「十一」中文编号或无 §11 章节号的旧大纲）。**约定**：大纲中除真正的 §11 精简段标题外，**其他任何标题不得同时以 `§11` 开头并含「精简段」/「写手版」字样**——否则将造成锚点错位与假 P0（实战：`## §D 图表建议（…§11 写手精简段引用）` 的**中置**「§11」曾触发旧版误命中）。
