> 版本：v2.5.2-dsh.16（DSH 原生插件，自动同步 2026-08-26）
...
>
> **机械化脚本化（v2.5.2-dsh 补丁，token 优化）**：M-Form 1-11 + M-Exist 1-10 + M-Integrity-1 是纯正则/结构判定，可由 `scripts/m-gate-check.mjs <final/定稿.md> <final/证据包/>` 脚本执行（**零 LLM 判定**）；T8 只判需判断力的 M-Form-8（三角验证）+ 复核脚本结果——省 T8 读定稿+证据包约 12K token，且比 LLM 自评更硬。
...
若无「承重墙」/「承重证据 top1」标记 → 记备注（不判失败；T6/T7 的清单专项检查会随之失效）
区块 = 该标记所在标题起、到下一个同级标题止（非标题行则取后续 ≤60 行）
结构性行 = 区块内含 [Lxx]/[Dxx]/[Cxx] 且（表格行 | 列表项 | 含「论点N」）的行
...
**为什么必须有这一项**：05 卡要求 T5「先读各卡索引段 → 按大纲映射表**只读相关条目，不读全文**」（T5 的 cacheRead 占子代理总量 ≈76%，整卡通读就是大头）。但「按需加载」与「整卡通读」在产物上**完全同形**——只看初稿无从区分，整条 token 优化的收益此前**全靠写手自述**。本项用一份 ≤30 行的留痕（`templates/素材加载清单-template.md`）把「到底加载了哪些编号」变成事实。
...
**为什么必须有这一项**：04/05 卡早已定案「T5 **只读**大纲末尾 §11 写手版精简段（≈60 行），完整大纲按需」——这条是 **T5 上下文 50K 大头的根治手段**（实测 T5 cacheRead 占子代理总量 76%）。但 §11 自身是否真含那六个要素（**论证主线 / 论点-论据映射表 / 反方规划要点 / 字数预算 / 禁做项 / 承重墙清单**）**从来没被核过**。风险很隐蔽：§11 缺要素时 T5 只能**回退整读大纲**，省 token 的机制**静默失效**，而产物上完全看不出来。
