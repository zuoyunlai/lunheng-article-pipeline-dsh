...
> - **P3** #13 实战反例结构化（教训库）
> - 全部 14 项反思见主控复盘报告（`<项目根>/run/审计/<内部报告名>.md`）
...
### exit 字段双语义定义（v18.0.0 新增，P0-2）

> **为什么必须定义**：旧版只写「exit 0 才允许返回」与「如实记账、不得伪造」两条，却**从未定义 `exit` 是「脚本机械输出」还是「T8 复核后的裁定值」**。两条规则同时成立时（存在**已知假阳性**时），T8 无合规出路——只能各自发明解法（实战：本项目 T8 临场加 `script_exit_raw` / `exit` / `exit_semantics` 三字段旁路），导致**口径无法跨项目比较**、交付说明的「M 门 exit」栏含义因人而异。

`M-Gate-Report.json` 必须含**两个**退出字段，语义严格区分：

| 字段 | 语义 | 可否修改 | 说明 |
|------|------|---------|------|
| **`script_exit_raw`** | `scripts/m-gate-check.mjs` 的**机械输出原值** | ❌ **禁止修改** | 不可篡改的机械证据；即使 T8 判定某 P0 为假阳性，也必须原样保留（这是「可复核」的基础） |
| **`exit`** | **T8 终检的最终裁定值** | ✅ 由 T8 判定 | `0` = 无真 P0/P1；`1` = 有真 P1；`2` = 有真 P0；`3` = 仅 P2·soft·SKIP（须 LLM 复核，**不得当通过**） |

**裁定值 ≠ 机械值时的强制留痕要求**（否则视为伪造）：

1. 必须新增 **`_t8_llm_review`** 段，对被裁定为「假阳性」的每一项给出**证伪证据四件套**：
   - **逐条枚举**（如「12 处命中 = `[D-基-T-01]`×7 + `[D-基-R-01]`×2 + `[D-基-E-01]`×2 + `[C-空]`×1」）
   - **真阳性扫描结果**（如「真占位符模式 `[待补]/[TBD]/_{3,}/[??]` 命中数 = 0」）
   - **规范冲突说明**（该模式由哪份 spec 强制保留，与脚本规则如何冲突）
...
>
> `_t8_llm_review` 的立法目的是处理**覆盖盲区 / 格式严格度 / best-effort 解析误判**，**不是**给结构性硬缺陷开后门。以下 4 类即使写了证伪四件套，也**必须真修复后才能 `exit ≠ 2`**；未修复而强行落 `exit=0` 视为伪造：
>
> | # | 红线项 | 判据 | 为什么不可兜底 |
> |---|---|---|---|
> | 1 | **M-Form-2 文末节缺失** | 必需五节（参考文献/数据来源/案例来源/先行者文献/AI 使用声明）任缺（学术四声明为**可选**，见 M-Form-7 九节口径） | 结构缺失 = 稿件不完整，不是判定口径问题 |
> | 2 | **M-Form-7 文末白名单/顺序错** | 非白名单节混入、或五节顺序颠倒 | 同上；且下游按顺序定位 |
> | 3 | **M-Exist-1 漏引 > 0** | 正文有编号而文末无条目 | 引用不可回溯 = F1 幻觉引用风险开门 |
> | 4 | **M-Integrity-1 数据不完整** | 数据条目数 < 任务简报「需找数据点」总数 | 证据底座不足，后续论证不可信 |
>
> **可兜底（正常走四件套）**：M-Form-4/5（术语口径冲突）、M-Form-8（锚点/布局/覆盖率严格度）、M-Form-9（启发式数字对账）、M-Form-10/11（best-effort 解析与比率档位）、M-Exist-5/6/7（报告格式严格度）。
>
> **⚙️ 实装状态（v18.12.0，2026-09-25 全量审计 L-44）**：本红线**此前只有规格、没有实现**——`m-gate-check.mjs` 的注释宣称「在 `exit` 判定时强制重审」，而紧随其后的 `exitCode` 表达式里一行红线判定都没有，四条红线仍可被一纸 `_t8_conclusion` 放行（审计夹具实测：删掉 `## 案例来源` 的稿子机械 `exit=2`，写入裁定段后落盘报告即变成 `exit=0`）。现落到三处：
> 1. `m-gate-check.mjs` 在 `exit` 判定时收集 **`hard_red_line_hits`**（命中四类中任一项且该项 `pass=false`）并写入报告；
> 2. 落盘时**拒绝采纳**既有 T8 裁定值（`sameDraft && hardRedLineHits.length === 0` 才保留），并置 `verdict_stale=true` + 打印 `⛔ 硬 P0 红线命中`；
> 3. `M-Exist-5` 的放行分支加前置条件 `hard_red_line_hits?.length === 0` —— 红线未修而强行落 `exit=0` **不再可能通过闸门对账**。
> 判据因此从「文档承诺」升级为「机器保证」：`hard_red_line_hits` 非空 ⇒ 任何 LLM 裁定都不被采信。
>
> **反面教训（本项目实测）**：T8 曾对 3 项真 P0（M-Form-4 元数据泄露 / M-Form-10 索引段 / M-Exist-1 硬孤儿 11 个）逐条写「假阳性」并落 `exit=0`；复核者两次判读均认定为「用 LLM 覆盖脚本值」。真修复后 `script_exit_raw` 由 **2 → 1**（P0 归零），字号也由 11,766 压回 11,513。**判据：兜底应当让 `script_exit_raw` 下降或保持，而不是让它停在 2。**
...
>
> **脚本是筛子，不是判别器（v18.0.0 新增）**：实战实测（本项目）22 项 M 门中曾出现 **10 项（45%）需 LLM 人工修正**（口径冲突 4 + 覆盖盲区 2 + 正则漏计 2 + 表解析脆弱 1 + 激活时序 1）。因此 **`script_exit_raw` 非 0 不等于存在真缺陷**；反之 **`exit 0` 也不保证稿件无瑕**（假阴性同样存在）。判别职责与成本由 T7/T8 显式承担，不得默认「脚本说通过就通过」。
>
...
>
> **路径前置提示（v18.0.0 新增，P1-2；发布前据回归用例修订为「告警不中止」）**：`m-gate-check.mjs` 在证据包目录存在但缺 `数据卡.md` / `文献卡.md` 时，**在 stderr 显著告警并给出正确用法**，然后**继续正常出报告**（不中止）。
>
> - **为什么当初想中止**：实战中主控误传 `analysis/`，脚本继续执行并一次产出 **8 个假 P0**（「数据卡.md 不在证据包」/「文献卡无对应条目」…），逐项读 detail 才能推断哪些是路径造成的。
> - **为什么最终改为告警**：① 本包契约是「**缺卡记 N/A、0 条场景合法**」（M-Form-10 用例明确「三张卡都没有 → N/A」）；② 证据包在 `build-evidence-bundle.mjs` 跑之前本就是空目录，中止会把「顺序没到」误报成「路径传错」；③ 中止（`exit 10`）会让调用方拿不到任何 JSON → `final-check.mjs` 与随包回归用例整体失效（实测 18 个用例红）。
> - **正确用法仍是**：第二参数传 `<final/证据包>`（不是项目目录、不是 `analysis/`），且先跑 `node scripts/build-evidence-bundle.mjs <run/项目名> --summary`。
> - **读到「无对应条目」类 P0 时的判据**：先看 stderr 有没有上面的告警——有则**先怀疑路径**，没有才是内容缺陷。
...
| `exist1ClosureP0` | 10 | M-Exist-1 漏引+孤儿 >N → P0（**保留原值**：v18.11.0 F-7 曾建议上调至 15，经回归测试驳回——`tests/scripts.test.mjs` 的 v18.3.1 B9「防降档」用例锁定 >10 条必须 P0） |
...
### M-Form-2: 文末白名单节存在性（v2.5.2-dsh.5 修订：与 M-Form-7 统一为 5 节 + v18.8.0 卷期页码扩展）

> **v18.8.0 扩展（借鉴 Ai4Scholar v2.9.4）**：启用 APA 优先输出时（任务简报 §v2.5.0 可选项 ✓ APA 优先输出），
...
> 触发条件：任务简报 §APA 优先输出勾选时启用；未勾选时 M-Form-2 v2 分支走 N/A 跳过（不影响 exit code）。
> 主控伪代码：见下文伪代码末尾 `if task_brief.apa_priority: check_vol_issue_page()` 块。
...
# 检查**必需 5 节**的存在性（与 M-Form-7 的**九节**顺序断言配套；v2.5.2-dsh.5 修订：原 4 节漏 AI 使用声明，
#   测试轮 v3 修 AI 声明前 M-Form-2 一直判过但 T7 打回——前后不一致，修订统一）
#   v18.12.0（全量审计 L-17）：四份学术声明（CRediT/COI/数据可用性/伦理审批）属**可选**（学术投稿才写），
#   故存在性仍只认这 5 节；白名单的成员资格与顺序由 M-Form-7 按九节口径判（真源 endnote_order）。
required_sections = [
    "## 数据来源",
    "## 案例来源",
    "## 参考文献",
...
> **v18.2.2（主人授权的机制修订）**：「AI 使用声明」节**整体豁免**文末二级扫描——该节的唯一职责就是披露 AI 参与，措辞由 `templates/AI-使用声明-template.md` 规定，与「文末不得含内部流水线信息」不冲突。实测：定稿在声明节写「…三档**承重**证据」命中禁词「承重」→ 假 P0。**豁免范围仅「AI 使用声明」一节**；`参考文献 / 数据来源 / 案例来源 / 先行者文献` 四节仍照扫（v18.0.0 加二级扫描的动因正是 `## 案例来源` 泄露角色名）。
...
**文末节二级扫描（v18.0.0 实装，v18.0.2 补入本文档——此前实现有、规格无）**：白名单剥离的初衷是「书目条目里的作者名/机构名不算泄露」，但它同时把**文末四节变成唯一可合法泄露内部信息的区域**（实测：`## 案例来源` 内曾出现「案例检索员」「spawn + 立即 Done」「任务简报 §五 第 55–56 行」，而脚本判通过，最终由 T7 逐行人工扫出）。现脚本对文末节做**二级扫描**：
- 只豁免「以数字或 `[D-基-x-NN]` 基线编号开头的书目条目行」；`[C-空]` 等空卡标记行**不豁免**；
- 其余行照常按禁止词/内部术语扫描（角色名、平台动作、版本注记、内部术语、内部路径）；
- 命中即 **P0**（与主扫描同档）。
> 结论：**「白名单只豁免『该位置可以存在』，不豁免『该位置的内容合规』」**（教训 #150，已同步至 05 卡「文末五节纯净自查」与交接模板）。
...
# 白名单**九节**（必需 5 节 + 可选 4 学术声明；deliverables.md「定稿文末白名单」）
#   v18.12.0（全量审计 L-17）：旧版只列五节 → v18.10.0 起写手卡要求的九节被 M-Form-7 判 P0（不可兜底红线）
#   → 学术稿无出口。现按「必需 + 可选」两层：存在性（M-Form-2）只认必需五节，白名单（M-Form-7）认九节。
whitelist = [
    "参考文献", "数据来源", "案例来源", "先行者文献",
    "CRediT 作者贡献声明", "COI 利益冲突声明", "数据可用性声明", "伦理审批声明",
    "AI 使用声明",
]
required_sections = ["参考文献", "数据来源", "案例来源", "先行者文献", "AI 使用声明"]
...
**顺序断言（v18.0.0 实装，v18.0.2 补入本文档——此前实现有、规格无）**：白名单只保证「成员资格」，但 `deliverables.md`「定稿文末白名单」同时规定了**固定顺序**（参考文献 → 数据来源 → 案例来源 → 先行者文献 → AI 使用声明）。脚本 `m-gate-check.mjs` 现同时断言顺序：**五节出现顺序与规范不符 → P1**（错序会让读者/下游脚本按固定序定位时取错节）。判定见脚本 M-Form-7 段的「五节顺序断言」实现。

### M-Form-8: 三角验证覆盖率检查（v2.3.7 论文三实战升级 + v2.5.2-dsh.5 按论点逐一核）

> **v18.2.2（主人授权的机制修订）**：承重墙清单锚点新增**「表格表头锚点」并置其优先级最高**——T4 常把承重墙做成三角验证表的**一列**（表头 `| … | 承重 top1 | … |`），而非独立小节。旧实现只认「以承重墙/承重证据/承重清单开头的标题行」与「行内式 承重证据 top1」，于是命中的是表上方的**纪律说明引用块行**，收块时又在**空行处即停** → `structRows=0` → 假报「承重墙清单无结构性条目」P1。三处修正：① 表头锚点优先；② 收块容忍 ≤3 行空行间隔；③ 锚点搜索跳过围栏代码块。
...
### M-Form-10: 索引段完整性（v2.5.2-dsh.17 新增 + v18.8.0 T3.5 auto_cite-补充.md 扩展）

> **v18.8.0 扩展（借鉴 Ai4Scholar v2.9.1）**：T3.5 auto_cite 预标注阶段（主人 Phase 0 显式开启）产出
...
> **v18.2.2（主人授权的机制修订）**：① **比率对账先取交集**——分子 = |已加载 ∩ 索引段|，越出索引段的编号另记软提示、不参与比率（实测出现过 `已加载 54 / 索引 52`，分子 > 分母 → 比率 >100% 仍被判「>90% 整卡通读」，不可复算）；② **「白读」软提示排除 `## 已跳过` 段已声明的编号**（清单明示跳过者不再报「上下文浪费」）。
...
> **v18.2.2（主人授权的机制修订）**：**诊断增强**——最高频的漏引根因不是「缺条目」，而是**「参考文献」节误用 GB/T 7714 的数字标签 `[1]..[n]`**：本门编号正则 `\[(L|D|C-主|C|先)\d+\]` **要求字母前缀**，`[1]` 一条都扫不到 → 文末字母编号集为空 → **正文全部 `[Lxx]` 被判「漏引」**（本仓库 v18.2.1 `规范-机械门对照表.md` 已记「首版无 [Lxx] → 漏引 17」，v18.2.2 全量测试再次踩到）。旧 detail 只报「漏引 N」，主控与写手易误判为「缺条目」而去补条目（治标不治本）。现检测该形态并在 detail **前置可操作提示**（「把 [N] 逐条改为 [Lxx]，按现有顺序一一对应，禁止重排顺序」）。**判定逻辑与严重度完全不变，只改提示文字。**
...
> **v18.2.2（主人授权的机制修订）**：**阶段感知**——被审正文位于 `drafts/`（Phase ≤4 审计场景）且证据包为空时，本项记 **N/A（pass=true）** 而非 P0。理由：证据包由 **T8 终检** 的 `build-evidence-bundle.mjs` 生成，T7 审计时它本就「尚未到期」；旧实现一律判「0 个 .md 文件 → P0」，该 P0 从 Phase 4 起挂在报告里、**T5 修订轮无法关闭**，最终只能写进 Acknowledged Limitations。同文件「证据包完整性前置提示」段已定过同一原则。**被审正文为 `final/定稿.md` 时维持原判 P0**。
...
> **v18.12.0 逐行绑定（2026-09-25 全量审计 L-02/L-03/L-22）**：伪代码中「**若 T7.5 全行 ✓** → 与报告对账」的**「全行 ✓」前置已取消**——改为对**每一个结论为 ✓ 的行**逐行对账（模板本身允许 ✗ / N/A，旧前置使最需要它的项目全部绕过）。新增三项绑定：
> - **M 门行**：实据里的 `exit N` 必须 == 报告的 `script_exit_raw`（不等 → **P0** 实据与产物不符）；且「有效裁定值」（T8 裁定段干净才认 0；另见 L-04 字段名兼容 `true_p0_count` 与 L-44 红线拦截）必须为 0（非 0 → **P0** 结论与报告自相矛盾）
> - **handoff 行**：实据必须给出 `exit ∈ {0,20,21,22}`（缺 → **P1**）
> - **路径**：实据引用的项目内路径（`audits/ drafts/ analysis/ literature/ data/ cases/ final/证据包|图件/`）必须真实存在（不存在 → P2 软提示；`final/` 根下与 `references/`/`scripts/` 不核，避免对 Phase 5 尚未生成的产物误判）
> - 两份记录**都没写** handoff exit → **备注**（信息性，不计 pass——见修订记录 §未做项）
>
> **修复前的五条空洞（同一处门、互不重叠，任一未修则整门无效）**：① 模板解析循环在 T2.5 表末写 `break`（退出**整个**扫描）→ `required['T7.5']` **恒为空**，「逐项都要有行」在空列表上空转；② 实据列只做**语法匹配**——15 行全填伪造 `exit 0` 也判「实据为机械证据」；③ 结论词**不与实据对账**——`exit=2` 的实据标 ✓ 照样过；④ 「全行皆 ✓」前置（见上）；⑤ 「不写 handoff exit = P1」的规则**从未被检查**（20 个真实项目只有 1 个真跑过）。①②③④ 本批已修，⑤ 降为备注。
> **回归网**：`tests/mexist5-binding.test.mjs`（8 例，含 4 条负向：漏项必报 / 实据与产物不符必 P0 / 有 ✗ 行仍须对账 / 红线命中不放行）。
...
### M-Exist-6: 审稿报告与期刊匹配（v2.5.2-dsh.17 新增 + v18.8.0 G15 引用匹配度扩展）

> **v18.8.0 扩展（借鉴 Ai4Scholar v2.9.4）**：任务简报 §引用数量与质量控制 字段启用时，
> 审稿报告 §引用匹配度 段须满足：
...
> **v18.2.2（主人授权的机制修订）**：**寻找「下一标题」时必须跳过围栏代码块**——精简段正文常整体包在 ```` ```markdown ```` 围栏里，围栏内的 `# T5 写手 v1 精简段（≤60 行）` 这类**一级标题**会被 `^#{1,lvl}\s` 命中 → 段落在第 2 行即被截断 → 实测假报「精简段 2 行 / 六要素实到 0/6」P0（真值：围栏内 48 行、六要素齐备）。围栏开合用行首 ``` 计数。

**检查对象**：`analysis/分析大纲.md` 的 **§11「写手版精简段」**（位于 F3 检查 / 缺口表之前）的**六要素齐备性**。

**为什么必须有这一项**：04/05 卡早已定案「T5 **只读**大纲 §11 写手版精简段（≈60 行，位于 F3 检查 / 缺口表之前），完整大纲按需」——这条把 T5 的**可裁剪读入口**压掉九成（**实测**：大纲全文 → §11 省**中位 85%**，12 个真实项目 n=12；T5 的 cacheRead 占子代理 单项目峰值 76% / 本机聚合 63.6%）。但 §11 自身是否真含那六个要素（**论证主线 / 论点-论据映射表 / 反方规划要点 / 字数预算 / 禁做项 / 承重墙清单**）**从来没被核过**。风险很隐蔽：§11 缺要素时 T5 只能**回退整读大纲**，省 token 的机制**静默失效**，而产物上完全看不出来。
...
> **定位口径（v18.8.0 登记）**：本项以**标题开头**含 `§11`（`^#{2,4}\s*§11`）且含「写手版 / 精简段 / 精简」的标题为锚（优先匹配），找不到时回退到宽松匹配（兼容「十一」中文编号或无 §11 章节号的旧大纲）。**约定**：大纲中除真正的 §11 精简段标题外，**其他任何标题不得同时以 `§11` 开头并含「精简段」/「写手版」字样**——否则将造成锚点错位与假 P0（实战：`## §D 图表建议（…§11 写手精简段引用）` 的**中置**「§11」曾触发旧版误命中）。
...
> **v18.2.2（主人授权的机制修订）**：**「需找数据点」计数改为分组取大**——`任务简报-template.md` 自身规定该短语写**两处**（研究问题段逐子问题各一条 + 数据需求段总量一条），旧实现**一律求和** → 实测 3+6+8（子问题）+ 25（总量）= **42**，而作者声明的真实需求是 **25** → 数据卡 32 条被判「32 < 42 条」→ T2.5 步骤 4 触发「T2 重检索」**假 P0**，M 门永不可 exit 0。现规则：`agg` = 不在「子问题」行上的需求量之和；`scoped` = 在「子问题」行上的需求量之和；**需求 = max(agg, scoped)**（取大即按更严一方对账，不产生假通过）；两组皆空 → 0。
