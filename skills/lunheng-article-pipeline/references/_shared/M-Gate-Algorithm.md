...
> **定位口径（v18.6.2 登记）**：本项以**标题开头**含 `§11`（`^#{2,4}\s*§11`）且含「写手版 / 精简段 / 精简」的标题为锚（优先匹配），找不到时回退到宽松匹配（兼容「十一」中文编号或无 §11 章节号的旧大纲）。**约定**：大纲中除真正的 §11 精简段标题外，**其他任何标题不得同时以 `§11` 开头并含「精简段」/「写手版」字样**——否则将造成锚点错位与假 P0（实战：`## §D 图表建议（…§11 写手精简段引用）` 的**中置**「§11」曾触发旧版误命中）。
