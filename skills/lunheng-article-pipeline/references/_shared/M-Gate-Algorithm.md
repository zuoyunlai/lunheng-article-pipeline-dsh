...
>
> **机械化脚本化（v2.5.2-dsh 补丁，token 优化）**：M-Form 1-11 + M-Exist 1-9 + M-Integrity-1 是纯正则/结构判定，可由 `scripts/m-gate-check.mjs <final/定稿.md> <final/证据包/>` 脚本执行（**零 LLM 判定**）；T8 只判需判断力的 M-Form-8（三角验证）+ 复核脚本结果——省 T8 读定稿+证据包约 12K token，且比 LLM 自评更硬。
...
3. ✅ **依次执行 M-Form 11 项 + M-Exist 9 项 + M-Integrity 2 项**（按本规约伪代码推理，含 v2.2.1.2/v2.2.4 升级算法）
...
## M-Exist 存在性合规门（9 项，含 v2.2.1.2 + v2.2.4 升级算法 + v2.5.2-dsh.17 审计条目闭环 / 闸门记录表 / 审稿报告与期刊匹配 / 交付说明字段 / 批判报告覆盖 / G 项覆盖）
...
3. M 门（M-Form 11 项 + M-Exist 9 项）全部 exit 0：读 M-Gate-Report.json → 全部 true
