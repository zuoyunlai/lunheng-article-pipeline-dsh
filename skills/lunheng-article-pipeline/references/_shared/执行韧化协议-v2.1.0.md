...
> **DSH 精简说明（v2.5.2-dsh.0）**：本文件为 OpenClaw 完整协议。DSH 环境下**仅执行精简版**——状态机（status.md 主控独占写）+ 交接报告六要素 + G8 自检 + 超时介入（`list_agents`）；**心跳/分阶段 ack/模型健康度预检/8 分钟硬卡不强制**（DSH 模型路由由 `settings.yaml` 决定，无脚本级 fallback 链）。完整内容保留作参考。
...
- **每 5 分钟一次**：在 status.md 阶段行末尾追加心跳时间戳（格式：（检查））
...
#### 4.1 spawn 后验证落地（必做）
每次 `subagent` 后，**必须**立即调 `list_agents()` 确认 runId 出现在 active runs，再进入 yield。没出现 = spawn 丢失，立即重试（≤2 次）：
```python
# 伪代码（spawn 验证）
run = subagent(task=...)
...
`等完成通知` 后超过 N 分钟（推荐 **3 分钟**，可根据 Phase 调整）仍无完成事件 → 主控**不再静默等**，自查 `list_agents` + `list_agents`：
...
> **v2.2.11 范围调整**：原 § 4.5 中的 spawn 日志诊断方法（读取 OpenClaw runtime 内部会话轨迹文件）原本在主流程中提及，但**该路径属于 OpenClaw runtime 内部诊断**（不在论衡主流程职责范围内）。
...
- `list_agents()` —— 看当前 session 有没有真的 spawn 出子代理、runId 在不在 active
- `list_agents` —— 列出可见 sessions（子代理是 `subagent:xxx` sessionKey）
