...
| T8 终检交付（**独立角色，v2.5.2-dsh.8 语义定案：由主控 T0 亲执行，不 spawn 子代理**） | ⬜ Inbox | | | | 含证据包（build-evidence-bundle.mjs 自动生成）+ M 门 20 项 + 终检必查项 15 项 + AI 使用声明 |
...
- 默认串联 3 脚本：count-chars.mjs（字数）+ m-gate-check.mjs（M 门 20 项）+ build-evidence-bundle.mjs --summary（证据包 + 审计视图）
