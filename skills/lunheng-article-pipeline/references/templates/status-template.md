...
| **G14 中文 AI 痕迹深度检测闸**（v2.4.0 新增；v18.2.8 删早闸）| ⬜ Inbox | | | | **三层防御**：① T5 v1 自检（写作时）→ ② 修订轮收尾自查（每轮修订后）→ ③ **终闸 Phase 4.5（与 T9 并行，唯一一次 spawn）**；命中 0-2 = Pass，3-4 = Warning 触发 T5 修订 1 轮，5+ = Fail 触发 T5 修订 2 轮 |
...
| T8 终检交付（**独立角色，v2.5.2-dsh.8 语义定案：由主控 T0 亲执行，不 spawn 子代理**） | ⬜ Inbox | | | | 含证据包（build-evidence-bundle.mjs 自动生成）+ M 门 23 项 + 终检必查项 15 项 + AI 使用声明 |
...
- [ ] **M-Exist-3**（正文 [Dxx] ↔ 数据卡条目 **引用闭环**；信任级别由 M-Form-6 + G12 承担，v18.2.6 审计修复）
- [ ] **G14 中文 AI 痕迹闸**（v2.4.0 增；v18.2.8 删早闸：T5 v1 自检 → 修订轮自查 → 终闸 Phase 4.5 与 T9 并行）
...
| 能力档 | 角色 | 档位（**不写厂商标识**——Phase 0 自检结果回填，可经 `LUNHENG_*` 覆盖） | **本轮实际** | 余额 |
|--------|------|-------------------|-------------|------|
| 检索 | T1 / T2 / T3 | `<LUNHENG_RETRIEVAL_MODEL>`（候选池见 `_shared/模型路由.md`） | 主控填 | ✅/⚠️/❌ |
| 分析写作 | T4 / T5 / T9 | `<LUNHENG_STRONG_MODEL>`（同上） | 主控填 | ✅/⚠️/❌ |
| 批判审计 | T6 / T7 / G14 | `<LUNHENG_AUDIT_MODEL>`（同上） | 主控填 | ✅/⚠️/❌ |
| 主控 | T0 | 会话模型（settings.yaml） | 主控填 | ✅/⚠️/❌ |
| 终检 | T8（独立角色） | 会话模型（settings.yaml） | 主控亲执行（不 spawn） | - |

> **v18.12.0（全量审计 L-31）**：本表此前**写死厂商模型名**（`deepseek-v4-flash` / `deepseek-v4-pro`），
> 与 `AGENTS.md`「**禁止写死厂商默认值**」、`SKILL.md:133`「本卡不写候选池与模型名」、
> `_shared/模型路由.md`「写死 deepseek-v4-pro 在多 provider 机器上都是错的」**直接冲突**——
> 而且本模板会被主控抄进项目根 `status.md`，等于把错的模型名写进交付物。现改为档位占位符。
