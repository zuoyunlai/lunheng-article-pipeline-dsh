...
> **核心铁律**：数据图表**禁止文生图**（数字不可控）；SVG 是文本，主控 LLM 直接用 `write` 生成（v2.5.2-dsh.5 修订：SVG 生成走 write 工具不跑 shell，非「零 exec 哲学」——主控另有白名单脚本）。图上所有数字必须来自数据卡，与正文 [Dxx]/[Cxx] 引用一致，生成后抽查核对。
...
2. **生成后抽查核对**：主控生成 SVG 后，逐个数字 检查 回数据卡核对，不一致 → 修正。
...
| 维度 | 数据图表（本模板） | 封面视觉（DSH：SVG/投喂/图像 MCP） |
|------|-------------------|--------------------------|
...
- 如需 PNG（公众号排版），**默认主人用 rsvg-convert 本地转换（零外发，符合论衡「主流程不跑 shell」约定——主人手动执行）**；如主人明确同意调用外部图像服务，主控才可用图像 MCP 转 PNG（注意：PNG 转 SVG 不影响数字精确性）
