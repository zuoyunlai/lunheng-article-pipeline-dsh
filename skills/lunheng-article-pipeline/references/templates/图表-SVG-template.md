...
| 维度 | 数据图表（本模板） | 封面视觉（DSH：SVG / 图像 MCP） |
|------|-------------------|--------------------------|
| 生成方式 | 主控 `write` 手写 SVG | SVG 矢量风（程序化，本地）或主人投喂；如需文生图另配图像 MCP（如 MiniMax image-01） |
| 数字可控 | ✅ 来自数据卡，100% 精确 | ❌ 数字不可控（封面无需精确数字） |
| 是否需主人同意 | ❌ 否（SVG 本地生成零外发） | ✅ 是（如走图像 MCP 调用外部服务，教训 #44） |
| fallback | 无（SVG 永远可用） | SVG → 主人人工上传 |
...
- 如需 PNG（公众号排版），主人可用 rsvg-convert 等工具转（注意：PNG 转 SVG 不影响数字精确性）
