...
6. **验证**：本修改走完后必 `grep` 关键词 + 结构性 grep（如本手册的「## 交接报告」所有角色卡齐整性）；改论衡机制/文档后额外跑两条门（v18.0.0 起为双门）：
   ...
   - `dsh-plugin-dev check` —— 包面静态门（15 项：patch 合法性 / `package.json` 元数据 / 多语 README 一致性 / 工程红线；**目标 0 fail / 0 warn**）
   ...
   > **收口批必跑「差集 + 反向核验」**（v18.15.0 定案，主人指示「固化为固定动作」）：仓库级脚本 **closeout-verify**（位于仓库根 scripts 目录，**不随包**）——宣布「修订已全部完成」**之前**必跑，与上面几道门并列。两步：① **差集**（审计报告的每个 `L-NN` 必须至少被一处修订记录/CHANGELOG 提及）；② **反向核验**（某梯队登记为「未做/延后/保留代价」的项，其后梯队必须回标）。判据：**差集查漏，反向核验查「记录陈旧」**——18.12.2 的「已全部落地」假陈述由 ① 抓到，18.12.3 的「差集为空=完成」假安心由 ② 抓到。⚠️ **边界**：本门只报「需人工回核」的项，**不判「已完成」**——语义那半必须逐条回代码/产物实测（把「被提到」当「做完」正是 18.12.3 的错误）。原理与两次自证见 `references/maintainers.md` §七；脚本属**仓库级**（不随包）。
...
**机械层（官方 CLI，随知识库分发）**：
- `dsh-plugin-dev check` —— 15 项静态门（patch 合法性 / `package.json` 元数据 / 多语 README 一致性 / 工程红线）；**目标 0 fail / 0 warn**
