...
Phase 5 终检     → T8 终检（独立角色，主控 T0 以 T8 身份亲完成 M 门：M-Form 9 / M-Exist 3 / M-Integrity 2，LLM 兜底）→ final/定稿.md + 证据包/ + 交付说明.md
```
...
- **M 门（v2.2.0+）**：终检前必读 `references/_shared/M-Gate-Algorithm.md`（**仅 T7/T8 读**——T1-T5/T9 不读，因 M 门 14 项中 13 项已脚本化为 `scripts/m-gate-check.mjs`（M-Form 1-9 + M-Exist 1-3 + M-Integrity-1 佐证），LLM 只复核 M-Form-8 的承重墙质量与 M-Integrity-2 跨文件判断），按伪代码执行 M-Form/M-Exist/M-Integrity（M-Form 9 项含 M-Form-7 定稿文末白名单 v2.3.5 + M-Form-8 三角验证 v2.3.7 + M-Form-9 图件闭环 v2.5.2-dsh.16），产出 `final/M-Gate-Report.json`，exit 0 才返回
