...
const PATCH_PATH_RES = [
  /^\*\*\*\s+(?:Update|Add|Delete|Move to)\s+File:\s*(.+?)\s*$/gim,
  /^(?:\+\+\+|---)\s+([ab]\/.+?)\s*$/gim,
]
/** 深度遍历上限：防畸形/深递归参数把 guard 拖死（超过即停止，不抛）。 */
const MAX_DEPTH = 6
...
function writtenPaths(args, depth = 0, out = []) {
  if (args === null || args === undefined || depth > MAX_DEPTH) return out
  ...
  for (const [k, v] of Object.entries(args)) {
    ...
    if (typeof v === 'string' && v.trim()) {
      if (PATH_KEYS.some((p) => p.toLowerCase() === lower)) {
        out.push(v.trim())
      } else if (PAYLOAD_KEYS.has(lower)) {
