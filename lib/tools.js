...
function describeScriptFailure(tool, r, scriptTimeoutMs, scriptMaxOutputBytes) {
  if (r.spawnError === 'EPERM' || r.spawnError === 'EACCES') {
    return (
      ...
      `（\`node <技能目录>/scripts/${tool === 'm-gate-check' ? 'm-gate-check' : 'count-chars'}.mjs …\`），` +
 ...
 */
export async function installLunhengTools(ctx, { skillRoot, defineToolFactory, report, scriptTimeoutMs, scriptMaxOutputBytes } = {}) {
  ...
  disposers.push(tools.register(defineTool({
    ...
    async execute(args, exec) {
      ...
      if (!j) {
        ...
        throw new Error(`${describeScriptFailure('m-gate-check', r, scriptTimeoutMs, scriptMaxOutputBytes)}（exit=${r.code}）：${clip(r.err || r.out, 300)}`)
      }
      return {
  ...
  disposers.push(tools.register(defineTool({
    ...
    async execute(args, exec) {
      ...
      const j = parseJson(r.out)
      if (!j) {
        throw new Error(`${describeScriptFailure('count-chars', r, scriptTimeoutMs, scriptMaxOutputBytes)}（exit=${r.code}）：${clip(r.err || r.out, 300)}`)
      }
